...
const uint16_t BUS_MOTOR_CMD_ID = 0x310;
const byte BUS_MOTOR_CMD_SIZE = 6;
...
PID trkXpid(&trkXinput, &trkXoutput, &pidSetpoint, trkXkpid[0][0], trkXkpid[0][1], trkXkpid[0][2], DIRECT);
PID trkYpid(&trkYinput, &trkYoutput, &pidSetpoint, trkYkpid[0][0], trkYkpid[0][1], trkYkpid[0][2], DIRECT);
///STAB parameter
int stbPosition[2];
...
//MOTOR parameter
const int motorXlimit = 16000;
const int motorYlimit = 8000;
...
void buttonHandler()
{
	...
	if (button.read()) {
		...
		if (movementState != a) {
			...
			if (movementState == ButtonClass::MOVE_TRACK) moveTrackEnd();
			else if (movementState == ButtonClass::MOVE_STAB) moveStabEnd();

			//init new movementState
			if (a == ButtonClass::MOVE_TRACK) moveTrackInit();
			else if (a == ButtonClass::MOVE_STAB) moveStabInit();
...
void busInit()
{
	...
	sendMotorMsg.can_id = BUS_MOTOR_CMD_ID;
	sendMotorMsg.can_dlc = BUS_MOTOR_CMD_SIZE;
...
void busRecv()
{
	...
	if (bus.readMessage(&recvMsg) == MCP2515::ERROR_OK) {
		...
		if (_id == BUS_JOYSTICK_ID) {
			...
			if (bitRead(recvMsg.data[5], 4)) js.zoom = JS_ZOOM_IN;
			else if (bitRead(recvMsg.data[5], 6)) js.zoom = JS_ZOOM_OUT;
			else js.zoom = JS_ZOOM_NONE;
			if (js.deadman) {
				//joystick's pan command
				a = recvMsg.data[0] & 0xF;
				i = recvMsg.data[1];
				if (a == 0) js.pan = i;
				else if (a == 4) js.pan = 0 - i;
				else js.pan = 0;
			...
			if (optZoomLevel > 2) optZoomLevel = 2;
...
void moveHandler()
{
	if (movementState == ButtonClass::MOVE_MANUAL) moveManHandler();
	else if (movementState == ButtonClass::MOVE_TRACK) moveTrackHandler();
	else if (movementState == ButtonClass::MOVE_STAB) moveStabHandler();
}
...
void moveManConversion(int x, int y)
{
	...
	if (abs(x) < jsXMid)
		...
		motorXspeed = constrain(map(abs(x), jsXMid, jsXMax, speedXMid, speedXMax), speedXMid, speedXMax);
	if (x < 0) motorXspeed = 0 - motorXspeed;

	if (abs(y) < jsYMid)
		motorYspeed = constrain(map(abs(y), 0, jsYMid, 0, speedYMid), 0, speedYMid);
	else
		motorYspeed = constrain(map(abs(y), jsYMid, jsYMax, speedYMid, speedYMax), speedYMid, speedYMax);
	if (y < 0) motorYspeed = 0 - motorYspeed;
...
void moveStabHandler()
{
	...
	if (stbXpid.Compute())
		motorXspeed = (long)stbXoutput;

	if (stbYpid.Compute())
		motorYspeed = (long)stbYoutput;
