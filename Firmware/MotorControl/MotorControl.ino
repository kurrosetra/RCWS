...
void ioInit()
{
	...
	pinMode(TRIGGER_EN_PIN, OUTPUT);
	digitalWrite(TRIGGER_EN_PIN, LOW);
...
void busRecv()
{
	...
	if (bus.readMessage(&recvMsg) == MCP2515::ERROR_OK) {
		...
		if (_id == BUS_MTR_CMD_ID) {
#if LED_ON_BUS_STATE
			digitalWrite(INDICATOR_PIN, !digitalRead(INDICATOR_PIN));
#endif	//#if LED_ON_BUS_STATE
			...
			digitalWrite(TRIGGER_EN_PIN, triggerHOT);
...
void busSend()
{
	...
	if (millis() > _busSendTimer) {
		...
		//send motorControl' status to bus
